...
#     return insert_new_template(current_user)
def insert_new_template(current_user):
...
#     return get_all_template(current_user)
def get_all_template(current_user):
    templates = mongo.db.templates
...
@token_required
def get_single_template(current_user,id):
    templates = mongo.db.templates
...
@token_required
def updtate_single_template(current_user,id):
    templates = mongo.db.templates
    template = templates.find_one({'_id': id})
    if not template:
        return jsonify({'message': 'Template not found!'})
...
@token_required
def delete_single_template(current_user,id):
    templates = mongo.db.templates
    template = templates.find_one({'_id': id})
    if not template:
        return jsonify({'message': 'Template not found!'})
